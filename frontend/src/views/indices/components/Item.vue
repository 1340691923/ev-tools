...
const iconClass = ref(['el-icon-arrow-right', 'el-icon-arrow-down']);
const tagClass = ref(['el-tag-blue', 'el-tag-orange']);
const sizeClass = ref(['item-cell item-control-cell', 'item-cell']);
const widthClass = ref(['el-input el-input--small']);

function querySearch(queryString, cb) {
  let queryDataTmp = JSON.parse(JSON.stringify(queryData.value))
...
function querySearch2(queryString, cb) {
  let queryDataTmp = JSON.parse(JSON.stringify(queryData2.value))
  if (queryString == undefined) queryString = ''
  if (queryString.trim() == '') {
    if (queryDataTmp.length > max) {
      cb(queryDataTmp.slice(0, max))
...
function getName(key) {
  const result = queryData.value.find(item => item.value === key);
